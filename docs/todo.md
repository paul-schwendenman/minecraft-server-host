...
- [ ] **World switching** ([plan](plans/world-switching-plan.md)): phase 1 picks the world at boot from an `ActiveWorld` EC2 tag (Terraform metadata tags plus a `minecraft-active.service` boot unit). Phase 2 switches while the server is running: a watcher on the instance that calls `minecraftctl world switch`. The controls app stays unchanged. Every world's jar and map are already on prod (checked 2026-09-24).
